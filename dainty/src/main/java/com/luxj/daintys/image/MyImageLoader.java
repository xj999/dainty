...
public class MyImageLoader {
    ...
    private MyImageLoader(BaseImageStrategy strategy) {
        mStrategy = strategy;
    }
     ...
     * 使用自定义策略
     */
    public static MyImageLoader getInstance(@NonNull BaseImageStrategy strategy) {
        if (mInstance == null) {
            synchronized (MyImageLoader.class) {
                if (mInstance == null) {
                    mInstance = new MyImageLoader(strategy);
                    return mInstance;
                }
            }
        }
